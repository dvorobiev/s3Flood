...
- [x] Профили нагрузки: `read-heavy`, `write-heavy`, `mixed-70-30`, паттерны `sustained|bursty` — реализовано.
- [x] Тест чтения из бакета (поток в `/dev/null`, без нагрузки на диск) — реализовано в профиле `write-heavy`.
- [x] Смешанные режимы (одновременный upload/download, общая очередь) — реализовано в профиле `mixed-70-30`.
