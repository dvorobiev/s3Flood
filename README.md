...
- **`Latency W:p50=X.Xs p90=Y.Ys`**: Задержки операций записи/чтения
  - `p50` (медиана) — половина операций выполняется быстрее этого времени
  - `p90` — 90% операций выполняется быстрее этого времени
  - ⚠️ **Важно**: При работе с файлами разного размера общие метрики латентности смешивают маленькие и большие файлы, что может быть не очень информативно. Для детального анализа используйте `report.json`, где есть статистика по группам файлов (ТОП10 маленьких/больших) с нормализованными метриками скорости (MB/s)
- **Аналитика файлов**: Детальная статистика по ТОП10 больших/маленьких файлов с нормализованными метриками скорости сохраняется в итоговый JSON-отчёт (`report.json`) для постфактум анализа. Это более информативно, чем общие p50/p90, так как учитывает размер файлов
