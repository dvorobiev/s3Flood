...
### Что нового в 0.11.0-beta

- **Цифрам можно доверять**: исправлены перцентили (p90 мог быть ниже медианы), добавлены p50/p90/p95/p99 латентности, оверхед запуска `aws` CLI замеряется и указывается в отчёте, запись метрик больше не тормозит воркеров.
- **Честный датасет**: несжимаемое содержимое и случайные размеры файлов внутри групп — хранилища с компрессией/дедупликацией больше не «читерят». Быстрый старый режим: `--fill zero`.
- **Прогрев**: `warmup_sec` — первые N секунд не попадают в статистику.
- **`report.json` v2**: `meta`, перцентили латентности, ошибки по типам, посекундный `timeline` для графиков.
- **Новый интерфейс**: дашборд на rich со спарклайнами RPS и бейджами фаз, рестайл меню, аналитический просмотр метрик (разбивка по размерам/endpoint, топ медленных операций).
