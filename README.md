...
```
Слева — локальная файловая система, справа — бакет. `Tab` — переключение панели, `Enter` на объекте — его версии (если на бакете включено версионирование), `F5` — копировать между панелями (в режиме версий — скачать конкретную версию), `F6` — откатить объект к выбранной версии, `F8` — удалить объект/версию, `r` — обновить, `q` — выход. Доступен также из интерактивного меню («⇄ Браузер бакета»).
