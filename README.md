...
- **`ops N files/s`**: Количество операций (файлов) в секунду
- **`Latency W:p50=X.Xs p90=Y.Ys`**: Задержки операций записи
  - `p50` (медиана) — половина операций выполняется быстрее этого времени
  - `p90` — 90% операций выполняется быстрее этого времени
- **Аналитика файлов**: Детальная статистика по ТОП10 больших/маленьких файлов сохраняется в итоговый JSON-отчёт (`report.json`) для постфактум анализа
