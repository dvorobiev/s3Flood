...
function Generate-File-List {
    Write-Log "Preparing 100 test files in `"$localTempDir`"..."
    ...
    Write-Log "Created $($fileList.Count) test files."
...
trap {
    Write-Log "Stopping signal received. Cleaning up..." -ForegroundColor Yellow
    Get-Job | Stop-Job -PassThru | Remove-Job -Force
    if (Test-Path $localTempDir) {
        Write-Log "Deleting temporary directory `"$localTempDir`"..."
...
Clear-Host
Write-Log "Starting S3 Load Test (Batch Upload/Download Algorithm)."
Write-Log "Press Ctrl+C to stop."

$rclonePath = Get-Command .\rclone -ErrorAction SilentlyContinue
if (-not $rclonePath) { Write-Log "ERROR: '.\rclone' not found." -ForegroundColor Red; exit }
$rcloneFullPath = $rclonePath.Source
Write-Log "Found rclone at: $rcloneFullPath"
...
while ($true) {
    ...
    for ($i = 0; $i -lt $fileList.Count; $i += $batchSize) {
        ...
        Write-Log "--- Processing Batch $currentBatchNum of $batchCount ($($batch.Count) files) ---" -ForegroundColor Cyan

        # --- UPLOAD BATCH ---
        Write-Log "[UPLOAD] Starting jobs for batch $currentBatchNum..."
        ...
        Write-Log "[UPLOAD] Jobs started. Waiting for batch to complete..."
        Get-Job | Wait-Job | Out-Null
        Write-Log "[UPLOAD] Batch $currentBatchNum uploaded successfully." -ForegroundColor Green
        Get-Job | Remove-Job
        
        # --- DOWNLOAD BATCH ---
        Write-Log "[DOWNLOAD] Starting jobs for batch $currentBatchNum..."
        foreach ($localFilePath in $batch) {
            $fileName = Split-Path -Leaf $localFilePath
            $s3Path = "$($rcloneRemote):$($bucketName)/$($fileName)"
        ...
        Write-Log "[DOWNLOAD] Jobs started. Waiting for batch to complete..."
        Get-Job | Wait-Job | Out-Null
        Write-Log "[DOWNLOAD] Batch $currentBatchNum downloaded successfully." -ForegroundColor Green
        Get-Job | Remove-Job
    }

    # --- STAGE 3: DELETE ALL FILES AFTER PROCESSING ---
    Write-Log "--- STAGE 3: DELETING all $($fileList.Count) files ---" -ForegroundColor Yellow
    foreach ($localFilePath in $fileList) {
        ...
        if (($fileList.IndexOf($localFilePath) % 10) -eq 0) {
            Write-Log "[DELETE] Deleting files... ($($fileList.IndexOf($localFilePath)) of $($fileList.Count))"
        }
        & $rcloneFullPath deletefile `"$s3Path`"
    }
    Write-Log "[DELETE] All files deleted successfully." -ForegroundColor Green

    # --- CYCLE END ---
    Write-Log "--- FULL CYCLE COMPLETE. Restarting in 15 seconds. ---" -ForegroundColor Magenta
