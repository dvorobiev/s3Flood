...
- **Редактор конфигов**: убран дублирующий блок сводки, значения подсвечиваются цветом, изменённые строки помечаются `*`, переключаемые поля (Yes/No, порядок, паттерн) меняются по одному Enter, а для endpoint'ов автоматически подставляется порт `:9080`, если он не указан.
